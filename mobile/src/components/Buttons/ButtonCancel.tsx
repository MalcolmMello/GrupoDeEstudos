...
const ButtonCancel = () => {
  ...
  function showToast() {
    ToastAndroid.show('Reunião desmarcada!', ToastAndroid.SHORT);
    setOpened(false);
  }
  ...
  return (
    <View className="">
      <TouchableOpacity
        ...
        <Text className="text-white font-medium">Desmarcar</Text>
      </TouchableOpacity>
      {opened && (
        <View className="bg-sky-500 h-48 w-80 p-4 absolute right-2 rounded-lg justify-between -bottom-2">
          ...
          <Text className="text-white">Você realmente deseja desmarcar essa reunião?</Text>
          <View className="flex-row justify-end gap-4">
            ...
            <TouchableOpacity className="bg-sky-600 rounded-md justify-center" onPress={showToast}>
              <Text className="text-white text-center px-4">Desmarcar</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
